...
def save_translated_file(translated_file, file_type, initial_name, target_lang):
    ...
    if file_type == "docx":
        ...
        translated_file.save(file_name)
    elif file_type == "txt" or file_type == "srt" or file_type == "vtt":
        with open(file_name, 'w', encoding='utf-8') as f:
            f.write(translated_file.getvalue())
    return file_name

def evaluate_model(models, csv_files):
    evaluation_metrics = {}
    for file in csv_files:
        df = pd.read_csv(file)
        for col1 in df.columns:
            for col2 in df.columns:
                if col1 != col2 and (col1, col2) in models:
                    X = df[col1]
                    y_true = df[col2]
                    y_pred = models[(col1, col2)].predict(X)
                    accuracy = accuracy_score(y_true, y_pred)
                    f1 = f1_score(y_true, y_pred, average='weighted')
                    bleu = sacrebleu.corpus_bleu(y_pred, [y_true])
