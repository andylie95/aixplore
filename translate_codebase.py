    ...
    for col1 in data.columns:
        for col2 in data.columns:
            if col1 != col2:
                ...
                # Use MarianMT model from Hugging Face
                model_name = f'Helsinki-NLP/opus-mt-{col1}-{col2}'
                ...
                for text in X_test:
                    ...
                    y_pred.append(translated_text)
                
                accuracy = accuracy_score(y_test, y_pred)
                f1 = f1_score(y_test, y_pred, average='weighted')
                bleu = sacrebleu.corpus_bleu(y_pred, [y_test])
                
                metrics_key = f"{col1} -> {col2}"
                evaluation_metrics.append({
                    'translation': metrics_key,
                    'accuracy': accuracy,
                    'f1_score': f1,
                    'bleu_score': bleu.score
                })
                
                models[(col1, col2)] = (model, tokenizer)
                
    return models, pd.DataFrame(evaluation_metrics)
