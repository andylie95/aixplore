            ...
            if shape.has_text_frame:
                for paragraph in shape.text_frame.paragraphs:
                    for run in paragraph.runs:
                        ...
                        if models:
                            translated_text = translate_with_model(original_text, 'auto', target_lang, models)
                        else:
                            translated_text = translate_with_google(original_text, target_lang, translation_dict)
...
def translate_csv(df, target_lang, translation_dict, models=None):
    """
    Translate a CSV file.

    Args:
        df: DataFrame object to translate.
        target_lang: Target language.
        translation_dict: Dictionary containing word/phrase translations.
        models: Dictionary of trained models (optional).
    ...
    for col in translated_df.columns:
        translated_df[col] = translated_df[col].apply(lambda x: translate_with_google(x, target_lang, translation_dict) if isinstance(x, str) else x)
...
def save_translated_file(translated_file, file_type, initial_name, target_lang):
    ...
    Args:
        ...
        file_type: Type of the file (docx, xlsx, pptx, srt, csv).
    ...
    if file_type == "docx":
        ...
        translated_file.to_csv(file_name, index=False)
    return file_name
